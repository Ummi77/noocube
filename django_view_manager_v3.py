...
class DjangoViewManager_V3 ():
    ...
    B. [28-01-2024] Необходимы какие-то изменения для проекта PRJ_021. Шаблон скопирован из django_view_manager_v2.py . Для прошлых проектов пока изспользовать
    предыдущие версии, пока не разберемся в чем дело для универсализации
    
    A. Отключена аинициализация VI. Базовая настройка выходного контекста self.view_basic_context_inicialization_through(), 
    так как она обращается к абсолютной БД облигаций в методе bmms.get_index_packages_df_BMMS() и поэтому не может находится 
    в NOOCUBE
    ...
    def __init__(self, request, viewSettings = {}):
        ...
        # print(f"PR_NC_217 --> self.viewSettings = ")
        # pp(self.viewSettings)
        ...
        if 'funcs_nicks' in self.viewSettings : # 
            self.funcs_nicks = self.viewSettings['funcs_nicks']
        else:
            self.funcs_nicks = {}
...
### ВСПОМОГАТЕЛЬНЫЕ МЕТОДЫ 
    ...
    def inicialize_functional_processing(self):
        ...
        if 'sort_settings' in self.viewSettings : # Форматеры
            sort_settings = self.viewSettings['sort_settings']
        else:
            sort_settings = {}
        ...
        if 'prepare_fparams' in self.viewSettings : 
            prepare_fparams = self.viewSettings['prepare_fparams']
        else:
            prepare_fparams = []
            
            
        
        if 'table_codes' in self.viewSettings : 
            tableCodes = self.viewSettings['table_codes']
        else:
            tableCodes = {}
            
        # Настрйоки для простого выходного  реьд-блока с данынми (не табличными или простые таблицы. Вообще в 
        # принципе тоже любые. Все зависит от динамич функции подготовки кода)
        if 'data_block_out' in self.viewSettings :
            dataBlockCodes = self.viewSettings['data_block_out']
        else:
            dataBlockCodes = {}
            
        
        if 'assoc_titles' in self.viewSettings :
        ...
        if 'filtration' in self.viewSettings : # Форматеры
            filtration = self.viewSettings['filtration']
        else:
            filtration = {}
            
            
            
        if 'pagination' in self.viewSettings : # Форматеры
            pagination = self.viewSettings['pagination']
        else:
            pagination = {}
        ...
        self.dicThrough['decor_kwargs'] = {
            ...
            'table_codes' :tableCodes,
            'data_block_out' :dataBlockCodes,
            'pagination' : pagination,
            # 'filtering' : ms.FILTERS_EXPR_TEMPLATES_FOR_FUNCS_BY_TYPE_V2_, # Общая для всех Views ( все фильтры хранятся в одной и той же константе)
            'filtering' : filtration,
            'sorting' : sort_settings, # 
            'formatters': fromaters,
            'prepare_fparams' : prepare_fparams, 
            'assoc_titles': assocTitles,
        ...
        # Первичная инициализация вложенных словарей в self.dicThrough['decor_kwargs'] по принципу JSON, которые потенциально могут быть и которые 
        # к данному моменту не заданы по умолчанию. присваиваем им пустые форматы. иначе будут возникать ошибки в тех местах. где они по стандарту в коде использованы
        
        #  настройки вложенных словарей по сортировке фрейма
        if 'default_setting_sort' not in self.dicThrough['decor_kwargs']['sorting']:
            self.dicThrough['decor_kwargs']['sorting']['default_setting_sort'] = {}
            
        if 'default_sort_col' not in self.dicThrough['decor_kwargs']['sorting']['default_setting_sort']:
            self.dicThrough['decor_kwargs']['sorting']['default_setting_sort']['default_sort_col'] = {}
    ...
    def view_basic_context_inicialization_through (self):
        ...
        # # НЕ УДАЛЯТЬ [закоменчено 07-04-2024] [Учасвовало в бондах]
    ...
    def get_dynamic_decor_func (decorNick):
        ...
        classObj = globals()['DjangoViewManager_V3'] # Нахождение класса в глобальных переменных  Get class from globals and create an instance
    ...
    def df_functional_processing(self, dicOfDecorFuncs, origTargetedDfQnStage = 'mask_delete'):
        ...
        print(f'-->  START PR_NC_125 --> : df_functional_processing()')
        ...
        for funcDecor in listFuncsOrd:
            ...
            oFunc = DjangoViewManager_V3.get_dynamic_decor_func(completeFuncname)
        ...
        print(f'-->  END PR_NC_126 --> : df_functional_processing()')
    ...
    def df_functional_processing_through(self, dicOfDecorFuncs, origTargetedDfQnStage = 'mask_delete', **fkwargs):
        ...
        1 : 'assoc_titles' : 'df_assoc_titles_and_calcs_dd_through_v3', # трансформация названий колонок процедурного фрейма
        2 : 'formatters' : 'df_formatting_dd_through', # форматирование колонок процедурного фрейма
        3 : 'mask_delete' : 'delete_df_rows_by_column_mask', # удаление рядов в зависимости от колонки-маски 
        4 : 'sorting' : 'df_sorting_dd_through', # сортировка по колонкам таблицы на странице сайта
        5 : 'filtration' : 'df_filt_dd_through', # фильтрация  процедурного фрейма
        6 : 'paginator' : 'df_paginator_dd', # пагинация процедурного фрейма
        7 : 'table_codes' : 'df_table_code_dd', # трансформация названий колонок процедурного фрейма
        8 : 'prepare_fparams' : 'df_prepare_fparams_dd_through', # постранияный форматинг
        
        """
        
        
        print(f'-->  START PR_NC_127 --> : df_functional_processing()')
        ...
        print(f"PR_B353 --> listFuncsOrd = {listFuncsOrd}")
        
        
        
        for inx, funcDecor in enumerate(listFuncsOrd):

            print(f"PR_NC_192 --> SYS LOG: {inx}. Текущий функционал из обоймы декораторов-обработчиков фрейма: {funcDecor}")
            
            completeFuncname = self.funcs_nicks[funcDecor]
            ...
            # Этап , после которого  нужно зафиксировать кол-во рядов процедурного фрейма для фиксации изначального кол-ва рядов в исходном целевом фрейме
            if funcDecor == origTargetedDfQnStage and not isinstance(self.dicThrough['df'], int):
                self.dicThrough['df_qn_orig'] = len(self.dicThrough['df'])
                
            
            
        
        print(f'-->  END PR_NC_128 --> : df_functional_processing()')
...
### ДИНАМИЧЕСКИЕ ДЕКОРАТОРЫ (DD) (ИЛИ ФУНКЦИОНАЛЫ)
    ...
    def df_assoc_titles_and_calcs_dd_through_v3(self, **fkwargs):
        """
        DjangoViewManager
        Динамический Декоратор-функционал для переименования колонок и добавления/изменения калькулируемых колонок
        
        ИЗМ v3: [28-01-2024]
        1/ Теперь используем не порядковый номер колонок, а их названия в константе ассоциаций. Порядок итак задается в ней. Калькулируемые колонки идут пока 
        с отрицательным int. Соотвтетсвенно, используем новый метод DSourceOutputCube_v2.diffirenciate_orig_and_calc_columns_static_v2()
        
        2/ Ранее ассоциативные словари задавались через порядковые ключи в виде целых чисел, которые означали порядок в таблице для колонок
        Теперь, начиная с PROJ_021 , эти ключи отменяются. Порядок задается самим порядком в ассоциативном словаре. А ключами теперь являются названия полей в исходном фрейме. Если фрейм формируется из таблиц, то тогда ключчами будут являтся названия колонок в таблицах БД, из которых формируется исходный фрейм 
        Еще раз, порядок вывода переименнованых колонок соотвтетсвует порядку полей в словаре ассоциаций 
        
        3/ Удаляется методы добавления калькулируемых колонок через словрь ассоциаций. Теперь все калькулируемые и форматируемые колонки формируются через форматтеры
        Удалены : DSourceOutputCube.diffirenciate_orig_and_calc_columns_static_v2 () и DSourceOutputCube.add_update_calculated_cols_static_v2_()
        Если их раскомментировать, то метод снова будет иметь возможность добавлять и калькулировать колонки из словаря-ассоциаций, как в ранних проектах. Но это более сложный и неприятный путь, чем форматтеры
        ...
        # Разделение оригинальных колонок и вычисляемых в два разных массива - закомментирован. Н опока оставляем некоторое время [28-01-2024]
        # origColsAssocTitles_, calcColsAssocTitles_ = DSourceOutputCube.diffirenciate_orig_and_calc_columns_static_v2(self.dicThrough['decor_kwargs']['assoc_titles'])
        
        
        
        
        
        # Переименовывааем названия колонок фрейма, а так же удаляем те, которых нет в словаре ассоциаций
        dicColsAssosiate = self.dicThrough['decor_kwargs']['assoc_titles'] # словарь ассоциаций
        
        print(f"PR_NC_216 --> SYS LOG: Словарь ассоциаций в обойме функционала: {dicColsAssosiate}")
        
        dicAssocLen = len(dicColsAssosiate)
        
        # print(f"PR_NC_193 --> dicAssocLen = {dicAssocLen}")
        
        if dicAssocLen > 0 and not isinstance(self.dicThrough['df'], int): # Проверяем, что в словаре ассоциаций есть колонки. которые надо переименовать
        
            self.dicThrough['df'] = PandasManager.rename_frame_columns_and_slice_by_dic_with_cols_names_stat_pandas (self.dicThrough['df'], dicColsAssosiate)
            
        else: # Если нет коллонок к переименованию, то пропускаем
            
            pass
        
        # НЕ УДАЛЯТЬ, Точка проверки
        # print(f"PR_NC_191 --> self.dicThrough['df'] = {self.dicThrough['df']}")
            
        
        # Оставляем пока как есть - добавление в конец вычисляемых колонок - закомментирован. Н опока оставляем некоторое время [28-01-2024]
        # fkwargs = {}
        # self.dicThrough['df'] = DSourceOutputCube.add_update_calculated_cols_static_v2_(self.dicThrough['df'], calcColsAssocTitles_, **fkwargs)
    ...
    def df_prepare_fparams_dd_through (self):
        """
        DjangoViewManager

        функционал подготовки массивов сквозных параметров для форматирования(если необходимо) проводится на этапе после пагинации
        Что бы массив параметров формировался только по фрейму выборки для страницы (а не на всем массиве. что при разростании БД будет выражаться в трате времени на обработку болших массивов)

        Category: Функционалы
        
        """
        
        print(f"PR_NC_196 --> START: df_prepare_fparams_dd_through()")
        ...
        if not isinstance(self.dicThrough['df'], int) and len(self.dicThrough['df'].index) > 0: # проверка наличия фрейма и так же наличия рядов в фрейме
            ...
            for wrapFaunc in self.dicThrough['decor_kwargs']['prepare_fparams']:
        ...
        print(f"PR_NC_197 --> END: df_prepare_fparams_dd_through()")
    ...
    def df_sorting_dd_through (self, **skwargs):
        ...
        if 'sort_col' in self.dicThrough['requestDic']:
            ...
            if 'default_sort_col' in skwargs: # Если задана колонка для сортировки по умолчанию
                ...
                # print(f"PR_NC_190 --> self.dicThrough['df'] = \n{self.dicThrough['df']}")
    ...
    def df_sorting_dd_through_v2 (self, **skwargs):
        """
        DjangoViewManager
        Динамический Декоратор-функционал для сортировки колонок процедурного фрейма
        ИЗМЕНЕНИЯ: 20-02-2024
        1. Отказ от skwargs (они уже не используются в функционалах). 
        2. Настройки по сортировки беруться только из settings.py из единого места
        3. Инициализация настроек по сортингу в скозном словаре производится  в inicialize_functional_processing() конструктора 
        в self.dicThrough['decor_kwargs'] = {... 'sorting' : ms.SORT_SETTINGS,  ...}. То есть верхний ключ в jSON ах является dicThrough['decor_kwargs'] и 
        дальше по цепочке
        RET: df
        Category: Декораторы
        """
        
        print(f'-->  START PR_NC_202 --> : df_sorting_dd_through()')
        
        # Выполняем только, если фрейм не пустой
        if not isinstance(self.dicThrough['df'], int):
        
            # INI
            if 'default_sort_col' in self.dicThrough['decor_kwargs']['sorting']['default_setting_sort']: 
                defaultSortCol = self.dicThrough['decor_kwargs']['sorting']['default_setting_sort']['default_sort_col']
            else: 
                defaultSortCol = False
            # Словарь для перевода стрингового True/False в булиновые значения
            toBool = {'True':True,'False':False}
            ...
            if 'sort_col' in self.dicThrough['requestDic']:
                ...
                # Сортировка по умолчанию {SORT_COLUMN_BY_DEFAULT}
                if defaultSortCol: # Если задана колонка для сортировки по умолчанию
                    requestSortCurrState['sort_col'] = defaultSortCol #dicDecorRes['df'].columns.tolist()[5] # 'Название' # df.columns.tolist()[0]
                else: # Если не задана дефолтная колонка для сортирвки, то сортировка идет по первой колонке с индексом 0
                    # print(f"PR_NC_190 --> self.dicThrough['df'] = \n{self.dicThrough['df']}")
                    requestSortCurrState['sort_col'] = self.dicThrough['df'].columns.tolist()[0]
            ...
            if 'sort_col' in requestSortCurrState and 'sort_asc' in requestSortCurrState:
                print(f"PR_NC_204 --> SYS LOG: Проводим сортировку фрейма в функционале: df_sorting_dd_through_v2() со следующими параметрами: \n sort_col : {requestSortCurrState['sort_col']}, sort_asc : {requestSortCurrState['sort_asc']} ")
                self.dicThrough['df'] = self.dicThrough['df'].sort_values(by=[requestSortCurrState['sort_col']], ascending = requestSortCurrState['sort_asc'])
                
                
        print(f'-->  END PR_NC_203 --> : df_sorting_dd()')
    ...
    def df_filt_dd (self):
        ...
        for key in kysIntersect:
            ...
            print(f"PROJ PR PR_NC_199 --> : Текущий ключ обоймы фильтров -> {key}")
    ...
    def df_filt_dd_through (self):
        ...
        print(f'-->  START PR_308 --> : df_filt_dd_through()')
        ...
        print (f"PR_NC_201 --> kysIntersect = {kysIntersect}")
        ...
        for key in kysIntersect:
            ...
            if len(self.dicThrough['requestDic'][key]) > 0:
                ...
                print(f"PR_NC_218 --> Текущее выражение фильтрации : {fExprByKey}")
                ...
                print(f"PR_461 --> df = \n{self.dicThrough['df']}")
                # Если разрешена фильтрация
                print(f"PR_NC_200 --> filterFlag = {filterFlag}")
                if filterFlag:
        ...
        print(f'-->  END PR_309 --> : df_filt_dd_through()')
    ...
    def df_paginator_dd_through (self):
        ...
        # # Выполняем только, если фрейм не пустой
        # if not isinstance(self.dicThrough['df']):


        # Если  есть ряды в фрейме
        if  not isinstance(self.dicThrough['df'], int) and len(self.dicThrough['df']) > 0:
    ...
    def df_table_code_dd (self):
        """
        OBSOLETED^ use df_table_code_dd_through
        DjangoViewManager
        Динамический Декоратор-функционал для фильтрации  процедурного фрейма
        RET: df
        Category: Декораторы
    ...
    def df_simple_data_block_html_code_out_dd_through (self):
        """
        DjangoViewManager
        Динамический Декоратор-функционал для создания динамического html-кода для станлдартного простого блока массивом данных, оформленным 
        динамически задаваемым методом
        RET: df
        Category: Декораторы
        """

        # Получить динамический обьект метода, отвечающего за формирование html кода для выходной таблицы
        oFunc = FunctionsGeneralClass.get_func_obj_by_its_full_name (self.dicThrough['decor_kwargs']['data_block_out']['prepareOutHtmlCodeFunc'], byClassFilePAth = True)
        # Подготовить выходной HTML-код для таблицы на бызе страницы процедурного (прошедшего все процедуры) фрейма
        htmlDataBlockOut = oFunc(**self.dicThrough)
        

        # V. обернуть tableCode , поступающий из предыдущих декораторов , сверху и снизу дивами со вставкой htnl- пагинатора
        # tableCode = self.dicThrough['tableCode']
        
        # Проверить флаг вывода пагинатора в конечном коде
        if 'outCodePagin' in  self.dicThrough and self.dicThrough['outCodePagin']:
            ...
            paginDecorator = paginDecorator.replace('%PAGINATOR_HTML%', self.dicThrough['paginatorHtml'])
            
            self.dicThrough['finalBlockCode'] = paginDecorator.replace('%DATA_BLOCK%', htmlDataBlockOut)
            
        else: 
            
            self.dicThrough['finalBlockCode'] = htmlDataBlockOut
...
if __name__ == '__main__':
    ...
    # ПРОРАБОТКА: переименование и слайс фрейма
    ...
    # DB_BONDS = '/home/lenovo/projects/P21_Telegram_Channel_Parsing_Django/telegram_channel_parsing_django/db.sqlite3' # LENOVO
    DB_TELEGRAM_LOADER = '/home/ak/projects/P21_Telegram_Channel_Parsing_Django/telegram_channel_parsing_django/db/telegram_ch_pars_django.sqlite3' # MSI

    # TODO: Подключение к БД происходит даже тогда, когда этого не нужно. нужно сделать так, что бы подключение к БД происходило исключительно в тех модулях, в которых необходимы работы с БД
    # Нет необходимости создавать подключение при импорте файла с установками. Продумать
    if hasattr(SqliteConnection, 'DB_CONNECTION'):
        print(f"PR_A098 --> SYS LOG: Обьект 'DB_CONNECTION' уже существует и нет необходимости его создавать")
    else:
        print(f"PR_A099 --> SYS LOG: Обьект 'DB_CONNECTION' НЕ существует и его необходимо создать")
        DB_CONNECTION = SqliteConnection(DB_TELEGRAM_LOADER)
        
    spps = SqlitePandasProcessorSpeedup(DB_CONNECTION)

    sql = f"SELECT * FROM tg_messages_proceeded JOIN tg_message_proceeded_ext ON tg_messages_proceeded.id = tg_message_proceeded_ext.message_proceeded_ref_id ORDER BY message_own_id DESC"
    ...
    dicColsAssosiate = COLS_ASSOC_FOR_BOOKS_DOWNLOADED_ # словарь ассоциаций
    ...
    print(f"listColsSlice = {listColsSlice}")
